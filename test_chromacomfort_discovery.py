#!/usr/bin/env python3
"""
ChromaComfort Discovery Test Script for Home Assistant SSH Terminal

This script tests the ChromaComfort integration discovery process with comprehensive logging.
Run this via SSH on your Home Assistant system to debug device discovery issues.

Usage:
1. Copy this file to your Home Assistant system (e.g., /config/test_chromacomfort_discovery.py)
2. SSH into Home Assistant
3. Run: python3 /config/test_chromacomfort_discovery.py
...
async def test_bluetooth_discovery():
    ...
    _LOGGER.info("=" * 80)
    _LOGGER.info("ChromaComfort Bluetooth Discovery Test")
    _LOGGER.info("=" * 80)
    ...
    try:
        ...
        for i, discovery_info in enumerate(all_discovered, 1):
            ...
            # Test ChromaComfort matching logic
...
def test_chromacomfort_imports():
    """Test if ChromaComfort integration files can be imported."""
    _LOGGER.info("-" * 60)
    _LOGGER.info("TESTING CHROMACOMFORT IMPORTS")
    _LOGGER.info("-" * 60)
    
    try:
        # Test if custom component exists
        chromacomfort_path = Path("/config/custom_components/chromacomfort")
        if not chromacomfort_path.exists():
            _LOGGER.error("ChromaComfort custom component not found at: %s", chromacomfort_path)
            return False
        
        _LOGGER.info("ChromaComfort component found at: %s", chromacomfort_path)
        
        # List files
        files = list(chromacomfort_path.glob("*.py"))
        _LOGGER.info("ChromaComfort files found: %s", [f.name for f in files])
        
        # Try importing
        sys.path.insert(0, str(chromacomfort_path.parent))
        
        try:
            from chromacomfort.config_flow import ConfigFlow
            _LOGGER.info("  ✓ config_flow.py imported successfully")
        except Exception as e:
            _LOGGER.error("  ✗ Failed to import config_flow.py: %s", e)
        
        try:
            from chromacomfort.coordinator import ChromaComfortCoordinator
            _LOGGER.info("  ✓ coordinator.py imported successfully") 
        except Exception as e:
            _LOGGER.error("  ✗ Failed to import coordinator.py: %s", e)
            
        try:
            from chromacomfort.const import DOMAIN
        ...
        _LOGGER.error("Error testing ChromaComfort imports: %s", e)
        return False

async def main():
    """Main test function."""
    _LOGGER.info("Starting ChromaComfort Integration Discovery Test")
    _LOGGER.info("Python version: %s", sys.version)
    _LOGGER.info("Working directory: %s", Path.cwd())
    _LOGGER.info("")
    
    # Test imports first
    imports_ok = test_chromacomfort_imports()
    _LOGGER.info("")
    ...
    _LOGGER.info("=" * 80)
    _LOGGER.info("ChromaComfort imports: %s", "✓ PASS" if imports_ok else "✗ FAIL")
    _LOGGER.info("Device discovery: %s", "✓ PASS" if discovery_ok else "✗ FAIL")
    
    if imports_ok and discovery_ok:
        _LOGGER.info("🎉 All tests passed! ChromaComfort should work with your device.")
    elif imports_ok:
        _LOGGER.info("⚠️  ChromaComfort files are OK, but no devices found. Check your fan is on and broadcasting.")
    else:
        _LOGGER.info("❌ ChromaComfort integration has issues. Check the errors above.")
